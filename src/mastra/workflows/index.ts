...
import { Client, GatewayIntentBits, type TextChannel, type ForumChannel, type ThreadChannel, type Message } from 'discord.js';
import { z } from 'zod';
...
async function getFirstThreadMessage({ client, threadId }: { client: Client, threadId: string }): Promise<string | null> {
  try {
    if (!BOT_TOKEN) {
