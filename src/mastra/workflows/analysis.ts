...
import { outputSchema, fetchForumPosts } from '../tools/fetchForumPosts';
...
const analyzeMessages = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    await fs.writeFile(filePath, JSON.stringify(postsForAgent, null, 2), 'utf-8');

    filePath = join(process.cwd(), '../../', 'categorized-posts-dump.json');
    await fs.writeFile(filePath, JSON.stringify(groupedPosts, null, 2), 'utf-8');
...
${summary
  .map(item => {
    ...
    const severityEmoji = item.severity === 'MINOR' ? '🟢' : item.severity === 'MAJOR' ? '🟡' : '🔴';
...
export const discordAnalysisWorkflow = createWorkflow({
  ...
  .then(createStep(fetchForumPosts))
